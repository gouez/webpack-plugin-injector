...
/**
 * Contains a collection of functions which sanitizes the plugin list for the website and administration
 */
class WebpackPluginInjector {
     ...
     * @param {String} section Either 'website' or 'administration'
    ...
    /**
     * Parses the plugin definition file and split them into the two sections website and administration.
     *
    ...
    getPluginConfig(pluginName, pluginDefinition, section) {
        ...
        if (pluginDefinition.website && section === 'website') {
            assetPaths.push(join(basePath, pluginDefinition.website.path, '../static'));
        }
    ...
    registerPluginsToWebpackConfig(plugins) {
        plugins.forEach((plugin) => {
            ...
            if (process.env.MODE !== 'hot' || this.section !== 'website') {
    ...
    injectCopyPluginConfig(plugins) {
        plugins.forEach((plugin) => {
            ...
            if (this.section === 'administration') {
                ...
                pluginPath = `${basePath}Resources/app/website/dist/website`;
                publicStaticPath = `${basePath}Resources/app/website/dist/static/`;
            }
